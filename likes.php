      ...
      if($select_likes->rowCount() > 0){
         while($fetch_likes = $select_likes->fetch(PDO::FETCH_ASSOC)){
            ...
            if($select_contents->rowCount() > 0){
               ...
               $select_mentors = $conn->prepare("SELECT * FROM `mentors` WHERE id = ?");
               $select_mentors->execute([$fetch_contents['mentor_id']]);
               $fetch_mentor = $select_mentors->fetch(PDO::FETCH_ASSOC);
   ?>
   <div class="box">
      <div class="mentor">
