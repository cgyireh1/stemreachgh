...
if(isset($_POST['like_content'])){
   ...
   if($user_id != ''){
      ...
      $mentor_id = $fetch_content['mentor_id'];
      ...
      if($select_likes->rowCount() > 0){
         ...
         $insert_likes = $conn->prepare("INSERT INTO `likes`(user_id, mentor_id, content_id) VALUES(?,?,?)");
         $insert_likes->execute([$user_id, $mentor_id, $content_id]);
...
if(isset($_POST['add_comment'])){
   ...
   if($user_id != ''){
      ...
      if($select_content->rowCount() > 0){
         ...
         $select_comment = $conn->prepare("SELECT * FROM `comments` WHERE content_id = ? AND user_id = ? AND mentor_id = ? AND comment = ?");
         $select_comment->execute([$content_id, $user_id, $mentor_id, $comment_box]);

         if($select_comment->rowCount() > 0){
            $message[] = 'comment already added!';
         }else{
            $insert_comment = $conn->prepare("INSERT INTO `comments`(id, content_id, user_id, mentor_id, comment) VALUES(?,?,?,?,?)");
            $insert_comment->execute([$id, $content_id, $user_id, $mentor_id, $comment_box]);
   ...
   <?php
      ...
      if($select_content->rowCount() > 0){
         while($fetch_content = $select_content->fetch(PDO::FETCH_ASSOC)){
            ...
            $select_mentor->execute([$fetch_content['mentor_id']]);
